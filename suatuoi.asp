...
' trang hien tai
    ...
    strSQL = "SELECT COUNT(MaSp) AS count FROM SanPham where LoaiSp ='Sua tuoi' AND TrangThai = '1'"
    connDB.Open()
...
<!-- #include file="header.asp" -->
    <section style="background-color: #eee;">        
        ...
        cmdPrep.CommandText = "SELECT MaSp,TenSp,LoaiSp,ThuongHieu,MoTa,Gia FROM SanPham where LoaiSp = 'Sua tuoi' and TrangThai = '1' ORDER BY MaSp OFFSET ? ROWS FETCH NEXT ? ROWS ONLY"
    ...
    <nav aria-label="Page Navigation">
                <ul class="pagination pagination-sm justify-content-center my-5">
                    ...
                    'kiem tra trang hien tai co >=2
                        if(Clng(page)>=2) then
                    %>
                        <li class="page-item"><a class="page-link" href="suatuoi.asp?page=<%=Clng(page)-1%>">Trước</a></li>
                    <%    
                        end if 
                        for i = 1 to range
                    %>
                            <li class="page-item <%=checkPage(Clng(i)=Clng(page),"active")%>"><a class="page-link" href="suatuoi.asp?page=<%=i%>"><%=i%></a></li>
                    <%
                        next
                        if (Clng(page)<pages) then

                    %>
                        <li class="page-item"><a class="page-link" href="suatuoi.asp?page=<%=Clng(page)+1%>">Sau</a></li>
                    <%
                        end if    
                    end if
                    %>
